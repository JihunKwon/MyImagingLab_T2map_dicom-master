...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Version 2.1
% modified on 09/13/2018 by Jihun Kwon
...
mkdir(strcat(dirname_crop,'\results'))
save(strcat(dirname_crop,'\results\t2map'),'t2map','S0map')
...
for i=1:size(t2map,3)
    for j=1:size(t2map,4)
        ...
        saveas(gcf,strcat(dirname_crop,'\results\map_s',num2str(j),'_tp',num2str(i),'.tif'))
    end
end
